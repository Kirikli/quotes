...
public class BashParser {
    public Map<Integer, String> getPage(int page) {
        ...
        try {
            ...
            for (Element quoteElement : sourceQuote) {
                int id = Integer.parseInt(quoteElement.select("b").first().text().substring(1));
                String quote = quoteElement.select(".quotbody").first().text();
                quotes.put(id, quote);
            }
    ...
    public Map.Entry<Integer, String> getRandom() {
        try {
            ...
            String quote = quoteElement.select(".quotbody").first().text();
            return new AbstractMap.SimpleEntry<>(Integer.parseInt(realId.substring(1)), quote);
    ...
    public Map.Entry<Integer, String> getById(int id) {
        try {
            ...
            String quote = quoteElement.select(".quotbody").first().text();
            return new AbstractMap.SimpleEntry<>(id, quote);
